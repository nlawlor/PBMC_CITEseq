...
ser <- readRDS("B.cells.no.doublets.no.IgM.seurat.Rds")
dim(ser)
meta <- ser@meta.data
...
pdf(file = "Bcell.baseline.antiCD3.plots.pdf", onefile = T)
...
saveRDS(pbmc, file = paste("Bcell.baseline.antiCD3.seurat.output.Rds", sep = "."))

dev.off()

pdf(file = "Bcell.baseline.antiCD3.dimplot.pdf", onefile = T)
...
pdf(file = "Bcell.baseline.antiCD3.clusters.pdf")
...
genes <- rev(c("ITGAX", "TBX21", "CD1C", "FGR", "FCRL5", "TFEC",
               "ZEB2", "ZBTB32", "CD86", "EBI3", "CXCR3", "CXCR5",
               "CR1", "TRAF5", "TLR7", "PRDM1", "CD27", "IFI27", "ISG15",
               "IFI44", "IFI44L", "STAT1", "MX1", "CD69", "CD83",
               "CCR7", "IL4R", "IL6", "FCER2", "CXCR4", "CD38",
               "CD5", "CD9"))
...
data_wide[,1] <- NULL
pdf(file = "Bcell.baseline.antiCD3.heatmap.pdf", onefile = T)
