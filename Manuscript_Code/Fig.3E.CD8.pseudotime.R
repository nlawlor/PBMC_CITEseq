...
pbmc <- readRDS("CD8T.doublets.removed.no.IgM.seurat.Rds")
...
mem_cd3 <- read.csv("CD8T.doublets.removed.Activated.Memory.vs.Unactivated.marker.genes.csv",
                     header = T, check.names = F, stringsAsFactors = F, row.names = 1)
mem_cd3_up <- mem_cd3[mem_cd3$avg_logFC > 0.25 & mem_cd3$p_val_adj < 0.05,]

naive_cd3 <- read.csv("CD8T.doublets.removed.Activated.Naive.vs.Unactivated.marker.genes.csv",
                     header = T, check.names = F, stringsAsFactors = F, row.names = 1)
...
pbmc_cont_naive <- dataset$cell_info[dataset$cell_info$Treatment == "Baseline" & dataset$cell_info$New_Annotations == "CD8T_Naive",]
pbmc_cont_mem <- dataset$cell_info[dataset$cell_info$Treatment == "Baseline" & dataset$cell_info$New_Annotations == "CD8T_Mem",]
pbmc_treat_naive <- dataset$cell_info[dataset$cell_info$New_Annotations == "CD8T_Naive_Actv", ]
pbmc_treat_mem <- dataset$cell_info[dataset$cell_info$New_Annotations == "CD8T_Mem_Actv", ]
